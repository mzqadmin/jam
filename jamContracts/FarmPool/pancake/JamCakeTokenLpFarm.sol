...
contract JAM is TokenWrapper,Ownable {
    using SafeMath for uint256;
    IERC20 public jam;

    uint256 public starttime;
    uint256 public daySeconds = 86400;
    uint256 public rewardRate = 7000 * 1e18 / daySeconds;  // ≈ 81018518518518512
    ...
    uint256 public tokenRewardPerTokenStored;
    mapping(address => uint256) public tokenUserRewardPerTokenPaid;
    mapping(address => uint256) public tokenRewards;
    string public rewardTokenName;

    // farm
    // Pancake Chef pool id index
    uint256 public cakeChefPid;
    uint256 public rateX = 1;
    address public strategyReward;
    uint256 public profitRadio = 3000;
    ...
    uint public maxUint256 = 115792089237316195423570985008687907853269984665640564039457584007913129639935;
    ...
    constructor(
        address jam_,
        address token_,
        address strategyReward_,
        uint256 cakeChefPid_,
        uint256 starttime_,
        ...
        rewardTokenName = rewardTokenName_;
    }

    modifier checkStart() {
        require(block.timestamp >= starttime, 'JAM: not start');
        _;
    }
    ...
    function tokenRewardPerToken() public view returns (uint256) {
        if (totalSupply() == 0) {
            return tokenRewardPerTokenStored;
        }
        return
    ...
    function tokenEarned(address account) public view returns (uint256) {
        return
        balanceOf(account)
        .mul(tokenRewardPerToken().sub(tokenUserRewardPerTokenPaid[account]))
        .div(1e18)
        .add(tokenRewards[account]);
    }
    ...
    tokenUpdateReward(msg.sender)
    checkStart
    {
        require(amount > 0, 'JAMPool: Cannot stake 0');
        uint256 newDeposit = deposits[msg.sender].add(amount);
        deposits[msg.sender] = newDeposit;
        // transfer from user address
        super.stake(amount);
        token.safeApprove(cakeChef, maxUint256);
        CakeChef(cakeChef).deposit(cakeChefPid, amount);
    ...
    checkStart
    {
        require(amount > 0, 'JAM: Cannot withdraw 0');
        deposits[msg.sender] = deposits[msg.sender].sub(amount);
        CakeChef(cakeChef).withdraw(cakeChefPid, amount);
    ...
    function getCAKEReward() public {
        CakeChef(cakeChef).deposit(cakeChefPid, 0);
    }
    ...
    checkStart {
        ...
        if (reward > 0) {
            if (profitRadio.div(FEE_DENOMINATOR) != 1){
                // get reward on pancake
                CakeChef(cakeChef).deposit(cakeChefPid, 0);
                // Cake at proportion
                uint256 tokenReward = tokenEarned(msg.sender);
                uint256 _strategyRewardAmount = tokenReward.mul(profitRadio).div(FEE_DENOMINATOR);
                uint256 _reward = tokenReward.sub(_strategyRewardAmount);

                // reward cake
                IERC20(cake).safeTransfer(strategyReward, _strategyRewardAmount);
                // user reward cake
                IERC20(cake).safeTransfer(msg.sender, _reward);
            }else{
                // get 100% reward if 100% of profitRadio
                IERC20(cake).safeTransfer(strategyReward, IERC20Check(cake).balanceOf(address(this)));
            }
